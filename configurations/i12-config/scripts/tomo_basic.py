...
#from org.edna.tomov1.xsdata import XSDataInputTomography
...
def basicTomoScanFromFile(filename):
    # get the bean from the 
    bean = XMLHelpers.readBean(File(filename),BasicTomographyParameters)
...
def basicTomoScanFromBean(bean):
    ...
    #scan(theta, scanStart, scanStop, scanStep, camera, expTime, ff, [flats,sectorSteps,darks], topup, 0)

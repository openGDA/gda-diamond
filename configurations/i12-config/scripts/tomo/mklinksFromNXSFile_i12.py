...
def makeLinksForNXSFile(\
	...
	if not imgkeyNXS:
		...
		if (inBeamPos is None) or (outOfBeamPos is None):
			raise Exception("INFO: Image-key data are not available in input NeXus file - please re-run this script with appropriate values following the stageInBeamPhys and stageOutOfBeamPhys options.")

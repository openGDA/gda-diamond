...
from i12utilities import i12tomoFlyScan
...
try:
    print "Attempting to find ss1_x..."
    ss1_x = finder.find("ss1_x")
    at = ss1_x()
    print "...found ss1_x at position: %s" %(at)
except:
    #ss1_x_dummy()   # will not print anything in Jython console
    print "Failed to find ss1_x_dummy"
...
#        if not indep_filenamepair_key in self.filename_fmt[funcname_key]:
 ...
 #       self.filename_fmt[funcname_key].append(dep_filename_val)
    ...
    def get_pairs_to_cp(self, fnname, fnum):
        # create pairs of (src, dst) for all filename templates
        _to_copy_fmt=[]
        for fmt_dct in nxs_audit.filename_fmt[fnname]:
            ...
            for p, lst in fmt_dct.items():
                ...
                if plen>1:
                    ...
                    _dst_indep = ""
                _src_indep_, _dst_indep_ = _handle_indep_fnames(_src_indep, _dst_indep, fnname)
                _to_copy_fmt.append((_src_indep_, _dst_indep_))
                for f in lst:
                    _src_dep = f.strip()
                    _dst_dep = _src_dep
                    _to_copy_fmt.append((_src_dep, _dst_dep))
                    
        # use filename templates to create filenames with actual file (scan) number
        _to_copy = []
        for p in _to_copy_fmt:
            _pe_lst = []
            for pe in p:
                #_pe_lst = []
                if len(find_sub('%', pe))==1:
                    ...
                    _pe_lst.append(pe)
            _to_copy.append(tuple(_pe_lst))
        return _to_copy_fmt, _to_copy

def copy_files(self, pairs4cp, dir_src, dir_dst, sleepdelta, niter, mode=0666):
    _success = True         
    for srcdst in pairs4cp:
        fname_src = srcdst[0]
        fname_dst = srcdst[1]
        ...
        if fpath_exists:
            ...
            _success &= copy_single_file(fpath_src, fpath_dst, overwrite=True)
            # enforce correct access permissions on dst_file
            #mode0666=0666
            try:
                os.chmod(fpath_dst, mode)
                msg = "Success in chmod to mode = %o for file %s" %(mode, fpath_dst)
                print msg
            except:
                exceptionType, exception, traceback = sys.exc_info()
                msg = "Failure in chmod to mode = %o for file %s" %(mode, fpath_dst)
...
nxs_audit.add_filename_fmt_dct( {'nxs_audit_tomo_step_scan': [{("%d.nxs",""): ["projections_%d.hdf"]}]} )
...
def nxs_audit_tomo_step_scan(outdirpath=None, runScan=True, scanNum=None, scanSubdir=None, sleepdelta=1, niter=30):
    """
    Desc:
    ...
    runScan - if True, tomoScan is run and then the resulting scan files are copied to dst dir; otherwise this 
        function will attempt to copy the scan files produced by the most recent scan that was run on the beamline
        Note: if False, this function will look for the most recent scan files in the directory reported by getSubdirectory(), 
            so the user is responsible for setting this sub-directory correctly (with the help of setSubdirectory) before 
            executing this function (please note that the most recent scan found in a particular sub-directory is 
            NOT necessarily the most recent scan on the beamline)
    scanNum - if runScan is False, then the scan files produced by this scan will be copied to dst dir instead of the
        most recent scan files
    scanSubdir - if runScan is False, then this sub-directory will be used to look for the scan files to be copied
    ...
    # use ss1_x_dummy for tomography_translation (defined in jythonNamespacemapping_live)
    # use ss1_theta_dummy for tomography_theta (defined in jythonNamespacemapping_live)
    # use pco4000_dio_hdf for tomography_detector (defined in jythonNamespacemapping_live)
    # use tomoScan command (other commands could be used, eg scan)
    # 
    overall_success = True
    subd_backup = getSubdirectory() # eg 'rawdata' as in /dls/ixx/data/2015/cm12345-1/rawdata/
    subd = "tmp"
    if runScan:
        ...
        try:
            tomoScan(scan_title, ss1_x_dummy(), ss1_x_dummy(), expoTime, \
		          startAng, stopAng, stepAng, \
		          darkFieldInterval=0, flatFieldInterval=0, imagesPerDark=2, imagesPerFlat=2, \
		          optimizeBeamInterval=0, pattern='default', tomoRotationAxis=0, addNXEntry=addNXEntry, autoAnalyse=autoAnalyse, additionalScannables=[])
    ...
    fnum = cfn() # int
    if (not runScan) and scanNum:
        fnum = int(scanNum)
    #dir_src = getVisitRootPath() + os.sep + (subd if runScan else getSubdirectory()) + os.sep
    dir_src = os.path.join(getVisitRootPath(), (subd if runScan else getSubdirectory()))
    if (not runScan) and scanSubdir:
        #dir_src = getVisitRootPath() + os.sep + scanSubdir + os.sep
        dir_src = os.path.join(getVisitRootPath(), scanSubdir)
    dir_dst = outdirpath
    if not dir_dst:
        dir_dst = nxs_audit.outdirpath[1]   # the one with trailing slash
    #dir_dst = "/dls/tmp/vxu94780/nexus_audit/i12/" # useful override for testing
    if (dir_dst in nxs_audit.outdirpath) and (not (nxs_audit.bl).lower() in dir_dst.lower()):
        dir_dst = os.path.join(dir_dst, nxs_audit.bl) # force-add final subdirectory 'ixx'; no trailing slash
    ...
    to_copy_fmt, to_copy = nxs_audit.get_pairs_to_cp(thisfn, fnum)  
    print "to_copy_fmt 1: ", to_copy_fmt
    print "to_copy 1: ", to_copy
    
    #copy_files(to_copy, dir_src, dir_dst, sleepdelta=sleepdelta, niter=niter, mode=0666)
...
def nxs_audit_tomo_fly_scan(outdirpath=None, runScan=True, scanNum=None, scanSubdir=None, sleepdelta=1, niter=30):
    """
    Desc:
    To run tomoScan and then copy the resulting scan files to the Nexus Audit dir or any specified output dir.
        Note: Depends on the presence of the nxs_audit object (defined at the top of this file) 
    ...
    niter - max number of attempts (iterations) for accessing each scan file (before a copy attempt to dst dir)
    """
    thisfn = nxs_audit_tomo_fly_scan.__name__
    print "Running %s..." %(thisfn)
    
    
    # use ss1_x_dummy for tomography_translation (defined in jythonNamespacemapping_live)
    # use ss1_theta for tomography_theta (defined in jythonNamespacemapping_live)
    # use pco4000_dio_hdf for tomography_flyScanDetector (defined in jythonNamespacemapping_live)
    # use i12tomoFlyScan command (other commands could be used, eg scan)
    # 
    overall_success = True
    subd_backup = getSubdirectory() # eg 'rawdata' as in /dls/ixx/data/2015/cm12345-1/rawdata/
    subd = "tmp"
    if runScan:
        ...
        #addNXEntry=True     # tomoFlyScan on i12 does it every time
        #autoAnalyse=False    # tomoFlyScan on i12 does it every time
        
        # run scan
        #def i13tomographyScan.tomoFlyScan(description, inBeamPosition, outOfBeamPosition, exposureTime=1, start=0., stop=180., step=0.1, darkFieldInterval=0., flatFieldInterval=0.,
        ...
        try:
            i12tomoFlyScan(scan_title, 0.0, 25.0, expoTime, \
                    startAng, stopAng, stepAng, \
                    imagesPerDark=2, imagesPerFlat=2, extraFlatsAtEnd=False, closeShutterAfterScan=True)
            print "Finished running the scan!" 
        except:
            exceptionType, exception, traceback = sys.exc_info()
            handle_messages.log(None, "Exception in %s" %(thisfn), exceptionType, exception, traceback, False)
    ...
    return overall_success    
"""
def nexus_audit_pixium():
    print "Running nexus_audit_pixium: NOT IMPLEMENTED"
    title_backup = getTitle()
    setTitle("nexus_audit")
    
    # run calibration?
    
    try:
        scan ix 0 9 1 pixium10_hdf 0.1
    except:
        exceptionType, exception, traceback = sys.exc_info()
        handle_messages.log(None, "Exception in nexus_audit_pixium", exceptionType, exception, traceback, False)
    finally:
        setTitle(title_backup)
    
    print "Finished nexus_audit_pixium"
    
def nexus_audit_edxd():
    print "Running nexus_audit_edxd: NOT IMPLEMENTED"
    # do sth
    print "Finished nexus_audit_edxd"
"""

def edxdBurtBackup(fileName, backupPath="/dls/tmp/vxu94780/"):
    #backupPath = "/dls_sw/i12/epics/burt/"
    os.chdir(backupPath)
    commandString = "burtrb -f edxd_preamps.req -o " + fileName
    print commandString

...
class PCOXgraph():
    def __init__(self, pv_prefix="BL12I-EA-DET-02", pv_hdf_component="HDF", name="pco_xgraph"):	# w/out CAM?
        ...
        self.visit = "ee19499-1"
        self.subdirpath = "rawdata"
    ...
    def collect_data(self, exposure_time_sec, acq_period_sec, nimages, subdirpath="raw", soft_terminated=False, prepped=False):
        ...
        outdirpath_template = "t:\\i12\\data\\\\2018\\%s" %(self.visit)
    ...
    #       #sleep(nimages*acq_per_rbv*(2.5))
            ...
            while ( (not self.abort_state) and caget(self.pv_prefix+":HDF:Capture_RBV")=="1"): # change this to comparing ints? 0=Done,1=Capturing
    ...
    def set_hdf5_writer(self, outdirpath, scan_number, nimages):    ## add filename_prefix_hdf and template? change to configure?
        ...
        caput_wait(self.pv_prefix+":HDF:Capture", 0) # Stop previous writing if any | 0=Done, 1=Capture
        
        #caputStringAsWaveform("BL13J-EA-DET-04:HDF5:FileName", self.filename_prefix)
        
        caput(self.pv_prefix+":HDF:EnableCallbacks", 1)
        caput_wait(self.pv_prefix+":HDF:FileNumber", scan_number, timeout=10)    #was 0
        
        caputStringAsWaveform(self.pv_prefix+":HDF:FileTemplate", self.file_template) #, timeout=10)
        caputStringAsWaveform(self.pv_prefix+":HDF:FileName", self.filename_prefix) #, timeout=10)    #caput has no effect
        caputStringAsWaveform(self.pv_prefix+":HDF:FilePath", outdirpath) #, timeout=10)
        
        self.hdfpath = self.file_template %(outdirpath+"/",self.filename_prefix,scan_number)
        print("hdfpath = %s" %(self.hdfpath))
        
        caput(self.pv_prefix+":HDF:AutoIncrement", 0)      #0=No, 1=Yes
        caput(self.pv_prefix+":HDF:AutoSave", 0)          #0=No, 1=Yes
        
        #caputStringAsWaveform("BL13J-EA-EXCBR-02:OD:FileExtension", "h5")
        
        caput(self.pv_prefix+":HDF:NumCapture", nimages)
        
        caput(self.pv_prefix+":HDF:FileWriteMode", 2)      #0=Single, 1=Capture, 3=Stream
        #blocking?
        
        sleep(1)
        caput(self.pv_prefix+":HDF:Capture", 1)    #0=Done, 1=Capture
        
        #return

    def stop_hdf_capture(self):
        caput(self.pv_prefix+":HDF:Capture", 0)

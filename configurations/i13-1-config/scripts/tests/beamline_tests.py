...
def checkScannable(obj):
    ok = True
    try:
        print "%s = %s" % (obj.getName(), `obj.getPosition()`)
    except:
        type1, exception, traceback1 = sys.exc_info()
        handle_messages.log(None,"checkScannable - error " + obj.getName(), type1, exception, traceback1, False)
        ok = False
    return ok
    ...
    for objectName in allObjectNames:
        ...
        try:
            ...
            if isinstance(obj, gda.device.Scannable):
                numScannables += 1            
                ok = checkScannable(obj) and ok
        except:
            type1, exception, traceback = sys.exc_info()
            handle_messages.log(None,"checkScannables - error getting attribute " + objectName + " item in namespace = " + objectNameOrig, type1, exception, traceback, False)
            ok = False
    if(numScannables != 8):
...
def get_names(namefile):
    ...
    for l in lines:
        if not l.startswith("#") or not len(l) == 0:

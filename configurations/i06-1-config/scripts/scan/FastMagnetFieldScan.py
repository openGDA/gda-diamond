...
class FastFieldScanController(object):
	""" """
		...
		def putCompleted(self, event):
			if event.getStatus() != CAStatus.NORMAL:  # @UndefinedVariable
				...
				if self.device.isScanAborted():
					logger.simpleLog('Epics Scan aborted. The FastFieldScanController calls back successfully')
				else:
					logger.logger.debug('FastFieldScanController.CaputCallbackListenerClass.putCompleted:Epics Scan finished. The FastFieldScanController calls back successfully. Delaying...')
					sleep(1)
					self.device.fixDataHead()
					logger.logger.debug('FastFieldScanController.CaputCallbackListenerClass.putCompleted:... Data Head fixed.')
	...
	def __init__(self, name, rootPV):
		...
		self.putListener = FastFieldScanController.CaputCallbackListenerClass(self)
		self.isCalled = True
...
#ScannableMotionBase Implementation
	def getPosition(self):
		mode = self.getIDMode()
		modeString = FastFieldScanController.FASTSCAN_MODE_GDA_STRING[mode]
		return modeString

	def asynchronousMoveTo(self,newPos):
		if newPos in FastFieldScanController.FASTSCAN_MODE:
			mode = newPos
		elif newPos in FastFieldScanController.FASTSCAN_MODE_EPICS_STRING:
			mode = FastFieldScanController.FASTSCAN_MODE_EPICS_STRING.index(newPos)
		elif newPos in FastFieldScanController.FASTSCAN_MODE_GDA_STRING:
			mode = FastFieldScanController.FASTSCAN_MODE_GDA_STRING.index(newPos)
		else:

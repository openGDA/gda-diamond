...
class EpicsLScontrol(ScannableMotionBase):
	...
	self.forcehrange=1 to force heater back on each time tset is read back (use watch if not in a scan). 
	self.waitafternewtemp=number of seconds to wait after tset is changed by more than 0.01 K (defalt is zero)
	Default is 0 (heater not forced on)
	...
	def __init__(self, name, pvstring, unitstring, formatstring,ch1,ch2):
		...
		self.speed=1
		self.forcehrange=0
		self.hrangesetval=self.hrange()
		self.waitafternewtemp=0

	def getPosition(self):
	#	sleep(1)
		Tset=float(caget(self.pvstring+'SETP'))
		if self.forcehrange==1:		#use only of heater keeps tripping off
			if self.hrangesetval!=0 and self.hrange()!=self.hrangesetval:
				print '===Heater tripped off - switching back to range '+str(self.hrangesetval)
				self.hrange(self.hrangesetval)
		return Tset
	...
	def asynchronousMoveTo(self,newT):
		...
		if abs(self.getPosition()-newT)>.01:######delete if problem
			sleep(self.waitafternewtemp)########delete if problem
	...
	def onRamp(self,speed):
		caput(self.pvstring+'RAMP_S',speed)
		caput(self.pvstring+'RAMPST_S','1')
		

	def offRamp(self):
		caput(self.pvstring+'RAMPST_S','0')
	...
	def hrange(self,xx=None):
		if xx!=None:
			self.hrangesetval=xx
			caput(self.pvstring+'RANGE_S',xx)
			sleep(1)
		self.hrangereadback=int(float(caget(self.pvstring+'RANGE')))
		return self.hrangereadback
	...
	def autotune(self,terange,channel):
		...
		for temp in temper:
			...
			while status==1:
				...
				status=int(caget(self.pvstring+'TUNEST'))
				sleep(5)	

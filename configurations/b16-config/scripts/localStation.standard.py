...
from gda.analysis.io.emulated import FileSystemEmulatingTIFFImageLoader
...
if installation.isLive():
	print "Installing micospiezo1/2 devices from epics BL16B-EA-PIEZO-03"
...
###############################################################################
print "Adding checkbeam device (rc>240mA, 60s wait after beam back)"
print "   (change threshold with checkbeam.minumumThreshold=12345)"
if installation.isLive():
	oldcheckbeam = pd_waitWhileScannableBelowThreshold.WaitWhileScannableBelowThreshold('checkbeam', rc, minumumThreshold=220, secondsBetweenChecks=1, secondsToWaitAfterBeamBackUp=180) #@UndefinedVariable
	oldcheckbeam = pd_waitWhileScannableBelowThreshold.WaitWhileScannableBelowThreshold('checkbeam', rc, minumumThreshold=220, secondsBetweenChecks=1, secondsToWaitAfterBeamBackUp=180) #@UndefinedVariable
...
if installation.isLive():	
	...
	checkrc = WaitWhileScannableBelowThreshold('checkrc', rc, 240, secondsBetweenChecks=1,secondsToWaitAfterBeamBackUp=5) #@UndefinedVariable
...
if installation.isLive() and ENABLE_PILATUS:
	print "-------------------------------PILATUS INIT---------------------------------------"
	try:
		pildet = EpicsPilatus('pildet', 'BL16B-EA-PILAT-01:',"",'p','%s%s%d.tif')
		visit_setter.addDetectorAdapter(PilatusAdapter(pildet, subfolder='pil100k'))

		pil = ProcessingDetectorWrapper('pil', pildet, [], panel_name='Pilatus Plot', toreplace=None, replacement=None, iFileLoader=PilatusTiffLoader, fileLoadTimout=60, returnPathAsImageNumberOnly=True)
...
if not installation.isLive():
	...
	ipp = ProcessingDetectorWrapper('ipp', ippws4, [], panel_name='ImageProPlus Plot', toreplace='N://', replacement='/dls/b16/data/')
...
print "Adding ai5 degfault detector"
print "Done!"

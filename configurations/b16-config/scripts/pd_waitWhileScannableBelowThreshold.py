...
class WaitWhileScannableBelowThresholdMonitorOnly(PseudoDevice):
	'''
	...
	'''
	def __init__(self, name, scannableToMonitor, minumumThreshold, secondsBetweenChecks, secondsToWaitAfterBeamBackUp):
		self.scannableToMonitor = scannableToMonitor
		self.minumumThreshold = minumumThreshold
	...
	def atScanStart(self):
		print '=== Beam checking enabled: '+self.scannableToMonitor.getName()+' must exceed '+str(self.minumumThreshold)
	...
	def getPosition(self):
		...
		if JythonServerFacade.getInstance().getScanStatus()==RUNNING:
			...
			if not currentStatus: # bad
				print "WaitWhileScannableBelow threshold not holding readback as no scan is running"
	...
	def getStatus(self):
		...
		if type(val) in (type(()), type([])):
			val = val[0]
		status =  (val >= self.minumumThreshold)
		return status
...
class WaitWhileScannableBelowThreshold(WaitWhileScannableBelowThresholdMonitorOnly):
	...
	def __init__(self, name, scannableToMonitor, minumumThreshold, secondsBetweenChecks, secondsToWaitAfterBeamBackUp):
		...
		WaitWhileScannableBelowThresholdMonitorOnly.__init__( self, name, scannableToMonitor, minumumThreshold, secondsBetweenChecks, secondsToWaitAfterBeamBackUp )

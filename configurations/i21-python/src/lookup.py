...
parser = argparse.ArgumentParser(description="Process ID lookupTable for (gap, row phase) <==> (polarisation, energy) in linear angular mode")
...
polarisationenergy_parser.add_argument('energy', metavar='energy', type=float, help='beam energy in eV, upper limit=1535eV, lower limit varies from 200eV to 450eV depending on polarisation')
polarisationenergy_parser.set_defaults(func=reverseLookup)

gaprowphase_parser = subparsers.add_parser('gaprowphase', help='lookupTable (gap, row phase) pair')
...
if __name__ == '__main__':
    ...
    if args.subparser_name == 'gaprowphase':
        if args.filename=='' or args.filename==None:
            ...
            print args.func(args.gap, args.rowphase, filename=args.filename)

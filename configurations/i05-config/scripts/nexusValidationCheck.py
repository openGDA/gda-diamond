...
arpes.APRESRun("/dls_sw/i05/software/gda/config/scripts/nexusValidationFixedScan.arpes").run() # editor.getPath()<-beanfile for swept scan <- variant of 
...
arpes.APRESRun("/dls_sw/i05/software/gda/config/scripts/nexusValidationSweptScan.arpes").run() # editor.getPath()<-beanfile for swept scan <- variant of 
lastScanFile = lastScanDataPoint().currentFilename                                             # get path of last file
...
# Notes
# APRESRun.run() ends up calling gda.jython.commands.ScannableCommands.staticscan([self.scienta]) after the scienta analyser is configured
# so excute APRESRun.run() config-only and then and extended command to exercise the nexus file

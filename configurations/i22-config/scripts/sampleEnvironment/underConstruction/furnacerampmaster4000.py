...
class FurnaceRampMaster4000(PseudoDevice, MonitorListener, Runnable, ScanPositionProvider):
    ...
    def __init__(self, name, furnace):
        ...
        self.fixOutput()
        self.setLevel(5)
        self.outcli=CAClient(self.furnace.pvbase+":STATUS")
    ...
    def nextRamp(self):
        ...
        if self.currentramp > 0 and math.fabs(targettemp - self.ramps[self.currentramp-1][1]) < 0.05:
            ...
            wthread.start()
            return
        self.furnace.setRamp(rate)
        self.furnace.setTemp(targettemp)
        #self.linkam.start()
    ...
    def calculateTime(self):
        temp = self.furnace.getPosition()[0]
        rampno = 0
        totaltime = 0 

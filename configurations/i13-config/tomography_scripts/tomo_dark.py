...
def setup_tfg(dfolder,exposure,nsteps,myfolder):
   global tfgscript
...
def doscan(dfolder,nsteps=1800,exposure=0.3,scanname="scan",endangle=180.0):
   global folderstr
   global linfolder
   global winfolder
   global trigpv
...
#**********************
   ...
   caput(trigpv,0,wait=True) #ensure trigger is at 0 to start with


   print("Arming the camera")
   caput("BL12I-EA-DET-02:CAM:ARM_MODE",1,wait=True,timeout=300)
   print ("waiting for camera to arm ... ")
   ...
   for ang in range(0,nplus ,1):
      ...
      print ("Triggering camera, theta = ", thisangle)
      #
...
class folders:
   ...
   def setbasefolders(self,sname):
      self.base="/data/2012/cm5706-1/processing/rawdata/"
...
if (len(sys.argv) < 4):
   print " Usage: %s <scan-name> <num-projections> <exposure-time-s> [<end-angle>]" % sys.argv[0]
   print "end-angle is optional (default 180.0 degrees)"
...
doflat(fpath,20,flat_exp,"df%s"%scanname)

print("calling doscan")
#camera.setup(exposure)
#doscan(fpath,nproj,exposure,scanname,endangle)

#tfgscan(fpath,nproj,exposure,scanname)

#print("calling doflat")
#camera.setup(flat_exp)
#doflat(fpath,20,flat_exp,"ff2%s"%scanname)

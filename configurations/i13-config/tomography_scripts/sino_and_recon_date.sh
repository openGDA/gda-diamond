#!/bin/bash
#set -x
#$Id: sino_and_recon.sh 95 2010-03-03 15:14:41Z kny48981 $
SCRIPTPATH=/dls_sw/i12/software/tomography_scripts
timestamp=`date +%y%m%d%H%M%S%N`
#set up some version tracking
svnid='$Id: sino_and_recon.sh 95 2010-03-03 15:14:41Z kny48981 $ '
...
jstring=`$SCRIPTPATH/sino_listener.py $* -U $mypid -i projections -o sino$datestring | tee out$datestring.txt |  grep "JOB NAME IS"`
rv=$?
...
set -x
$SCRIPTPATH/recon_array.sh $*  -i sino$datestring -o im$datestring -U $mypid -H $nm
set +x

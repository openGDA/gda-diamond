#!/bin/bash
#$Id: qunstack.sh 214 2012-02-13 15:46:36Z kny48981 $
#set up some version tracking
svnid='$Id: qunstack.sh 214 2012-02-13 15:46:36Z kny48981 $ '
svnurl='$URL: file:///home/kny48981/SVN/progs/tomo_recon_pipe/scripts/qunstack.sh $ '
...
success=0
nstacks=`(ls -1 $inputfolder/${basename}*.tif && success=1) | wc -l`
echo '$? = ' $?
echo 'success = ' $success
echo "$nstacks stacks"
if [[ $nstacks -eq 0 ]]
then
   echo "No tiff stacks called ${basename}.tif were found!"
   exit
fi
...
mysecs=`date +%s`
echo $mysecs
myscript=` printf "runqunstack_%d_%d.sh" $$ $mysecs `
echo $myscript
...
tiff_stack_stamp  -i $inputfolder -b $basename  -o $outputfolder -f \$thisfirst -n \$thisnum  -S $nimages -v -s \$logname -w /dev/null -x 
finishdate="\`date\`"
finishsec=\`date +%s\`

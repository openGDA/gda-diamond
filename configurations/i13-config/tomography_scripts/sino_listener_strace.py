...
#$Id: sino_listener.py 114 2010-08-25 17:16:00Z kny48981 $
import getopt
import sys
import os
import commands
...
def usage():
   svnstring="$Id: sino_listener.py 114 2010-08-25 17:16:00Z kny48981 $"
   global interval
   global lt
   print("%s version %s" %(sys.argv[0],svnstring))
   print("Usage:")
   print(sys.argv[0])
   print ("-i input dir (REQUIRED)")
   print ("-o output dir (REQUIRED)")
   print ("-w width (default 4008)")
   print ("-l length (height) of each chunk(default 167)")
   print ("-f number of first chunk (default 1) ")
   print ("-n number of last chunk (default 16) ")
   print ("-J job hame ")
   ...
   print ("-p number of projections per segment (REQUIRED)")
...
def main():
   ...
   try:                                
      opts, args = getopt.gnu_getopt(sys.argv[1:],  "U:O:C:EGI:J:N:R:STZ:b:cdf:hi:j:l:n:o:p:s:vw:xz:")
   ...
   settingsfolder="sino_settings/sino_job_%i" % mypid
   nproj=nperseg*nsegs
   ht=chunk_ht * nchunks
   projnum=0
   size=wd*ht
...
#      os.mkdir(outdir)
   ...
   if not(os.access ("sino_settings",os.F_OK)):
      os.mkdir("sino_settings")
   ...
   chunkprogram="sino_chunk_ezca_tiff_q" #use the path ... 
   chunkflags="-i %s -o %s -w %i -l %i -z %i  -Z %i  -s %i -p %i -b %i %s" % (indir,outdir,wd,chunk_ht,interval,lt,nsegs,nperseg,bytes,existflag) 
   myqueue="medium.q"
...
ulimit -c unlimited
# env > task${mytask}.env
tracename=tr${mypid}_j${myjob}_t${mytask}.trace

#trace execution all
strace -e file,desc -o ${tracename}  %s %s -m $mytask  -v -J %s${myjob}_t${mytask}
retval=$?
if [[ retval -ne 0 ]]
then
...
""") % (mypid,outdir,chunkprogram,chunkflags,jobname)
...
""") % (mypid,jobbasename)
   ...
   vprint (len(qenviron))
   vprint (qenviron.items())
   pyerr=open("sino_listener_stderr_%s.txt" % jobname ,"w")
   pyout=open("sino_listener_stdout_%s.txt" % jobname ,"w")
   pyenv_o=open("pyenv_o_%s.txt" % jobname,"w")
   pyenv_e=open("pyenv_e_%s.txt" % jobname,"w")
  ...
  #    sys.exit(147)
   try:
         ...
         thispid=os.spawnlpe(os.P_WAIT,"qsub","qsub", "-P","i12","-e",settingsfolder, "-o", settingsfolder, "-q",myqueue,"-N",jobname,"-cwd","-pe","smp","4","-t","%i-%i" % (firstchunk,nchunks),chunkscript, qenviron)

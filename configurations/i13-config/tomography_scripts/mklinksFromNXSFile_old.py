...
def estimateCOR(projFilename_0deg, flatFilename_0deg, projFilename_180deg, flatFilename_180deg):
...
def makeLinksForNXSFile(\
	...
	*additional
	...
	try:
		...
		print msg1
		msg2="Falling back on generating image-keys from available data.\n"
		print msg2
		pass
		...
		if sino_success and recon:
			...
			with cd(head+os.sep+sino_dir):
				try:
					...
					projFilename_0deg = srcfile_proj=tif[ proj_idx[0] ][0]
					flatFilename_0deg = srcfile_proj=tif[ flat_idx[0] ][0]

					#projFilename_180deg = srcfile_proj=tif[ proj_idx[len(proj_idx)-1] ][0]
					projFilename_180deg = srcfile_proj=tif[ proj_idx[len_proj_idx_decimated-1] ][0]

					flatFilename_180deg = srcfile_proj=tif[ flat_idx[0] ][0]

					COR=estimateCOR(projFilename_0deg, flatFilename_0deg, projFilename_180deg, flatFilename_180deg)
					COR=1557.8
					#launchReconArray(outDir=reconDir, ctrCoord=COR)
				except Exception, ex:

#!/bin/bash 
#$Id: flat_array.sh 214 2012-02-13 15:46:36Z kny48981 $
timestamp=`date +%y%m%d%H%M%S%N`
#set up some version tracking
svnid='$Id: flat_array.sh 214 2012-02-13 15:46:36Z kny48981 $ '
svnurl='$URL: file:///home/kny48981/SVN/progs/tomo_recon_pipe/scripts/flat_array.sh $ '
...
echo " -U specify unique identifier (default timestamp) "
echo " -a naverage - how many images to average (REQUIRED)"
echo " -n nchunks - how many of the available chunks to process"
echo " -H auto hold for prior job "
echo " -J jobname - short mnemonic name to include in the job name"
echo " -j suffix - job suffix name -- defult is script unique id "
...
echo " -p projections per segment (default 1)"
echo " -s segments per tomography dataset (default 1)"
...
echo " -G use Tesla GPU  (i.e. submit to tesla queue) "
...
echo "Submitting a flatfield averageing batch job"
...
logname=\`printf "fl_%s_t%s" \$myjob \$i\`
...
flat_chunk_capavg.q  -w $wd -l $ht -i $chunkdir -J \$logname -o $outdir -p 1 -s 1 -a $navg -m \$tasknum 

EOF
  #sumbit the "flat" job
flatname="fl_${jname}"
echo "Submitting the 'flat' job... $flatname"
qstring=`qsub ${qpflag} -t $firstchunk-$nchunks -q $qqueue  -cwd -pe smp 4 -N "${flatname}" runflatq.qsh`
retval=$?
echo "qstring = $qstring"
echo "Flat script returned: " $retval
...
qout=(${qstring})
qtask=${qout[2]}
echo "queue flat task number: ${qtask}"
echo "queue flat job name: ${flatname}"

IFS=$OIFS
if [[ $retval -ne 0 ]]
then 
  echo $qstring
  echo "ERROR: The 'flat' script returned a bad status for the queue submission."
...
echo "Your flatfield task $qtask from script with unique-id $mypid has been submitted." 
fi

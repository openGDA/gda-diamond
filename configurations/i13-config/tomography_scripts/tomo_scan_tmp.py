...
from getwaveform import getwaves
...
gstartmovetime=0 #global - time that continuous rotation has started
glastmovetime=0
gsteptime=0
gncalls=0
gnbad=0
...
def imageinccb(value):
      global gstartmovetime
      global glastmovetime
      global gsteptime
      global gncalls
      global gnbad
      global gmessages
      ...
      thismovetime=time.time()
      elapsed = thismovetime-glastmovetime
      angle=caget("BL12I-MO-TAB-02:ST1:THETA.RBV")
      if (gncalls > 2):
         ratio=elapsed/gsteptime
         if (ratio >= 1.5):
            gmessages.append("imageinc: large step found -- time = %f elapsed = %f gsteptime = %f ratio = %f angle= %f ncalls= %i nbad= %i" % (thismovetime,elapsed,gsteptime,ratio,angle,gncalls,gnbad))
            getwaves("%s/w_%05d.dat"%(gwavpath,gnbad))
            gnbad=gnbad+1
      glastmovetime=thismovetime
      gncalls=gncalls+1
...
def checkwrite(messages,looptime=0):
   ...
   if (tries >= 10 ):
      messages.append("Stopping file saving due to lack of response\n" )
      print "Stopping file saving due to lack of response\n" 
...
#**********************
   ...
   stopplus=stop+(0.6 * step)
   print("Setting up position compare, start = %f, stopplus = %f, step = %f" %(start,stopplus,step))
   ...
   if(maxspeed < slowspeed):
      ...
      print("Requested speed ", slowspeed," calculated step ",step,"  requested exposure ",exposure," and readout ",readout)


   gsteptime=float(step)/float(slowspeed)
   ...
   print("checking if camera is ready")
   busyv=caget(busypv)
   tries=0
   checksleep=1.0
   ...
   stime=time.time()
   gstartmovetime=stime

   cahandle=camonitor("BL12I-EA-DET-02:CAM:NumImagesCounter_RBV",imageinccb)

   caput("BL12I-MO-TAB-02:ST1:THETA.VAL",thetapos,wait=True,timeout=outtime)
   cahandle.close()
   etime=time.time()

   for mm in range(0,len(gmessages)):
      messages.append("%s\n"%gmessages[mm])
   ...
   print ("Writing Log File %s" %logname)
...
#   cothread.Sleep(exposure/2+0.55)
   ...
   for ang in range(0,nplus ,1):
      ...
      myang = startang + ang *  (endangle / float(nsteps))
...
doposcompscan(fpath,nproj,exposure,scanname,startangle=startangle,endangle=endangle)

#print("calling doflat")
#camera.setup(flat_exp)
#doflat(fpath,10,flat_exp,"ff2%s"%scanname)

#doscan(fpath,nproj,exposure,"step%s"%scanname,endangle=endangle)

#print("calling doflat")
#camera.setup(flat_exp)
#doflat(fpath,10,flat_exp,"ff3%s"%scanname)

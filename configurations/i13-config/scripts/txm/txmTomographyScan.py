...
def make_tomoScanDevice(tomography_theta, tomography_shutter, tomography_translation, 
                        image_key, tomography_imageIndex, use_slits=True):
    tomoScanDevice = ScannableGroup()
    tomoScanDevice.addGroupMember(tomography_theta)
    if use_slits:
        tomoScanDevice.addGroupMember(tomography_shutter)
    else:
    ...
    tomoScanDevice.setName("txmTomoScanDevice")
    tomoScanDevice.configure()
    return tomoScanDevice


class   tomoScan_positions(ScanPositionProvider):
    def __init__(self, start, stop, step, imagesPerDark, flatFieldInterval, imagesPerFlat,
...
#        self.darkFieldInterval = darkFieldInterval
    ...
    def __str__(self):
        return "Start: %f Stop: %f Step: %f imagesPerDark:%d Flats every:%d imagesPerFlat:%d InBeamPosition:%f OutOfBeamPosition:%f numImages %d " % \
            ( self.start, self.stop, self.step,self.imagesPerDark, self.flatFieldInterval, self.imagesPerFlat, self.inBeamPosition, self.outOfBeamPosition, self.size() ) 
    def toString(self):
        return self.__str__()
...
#                 raise ValueError("Unable to find data in file"+hint)
# 
...
def getTotNumScanPoints(nPoints, flatFieldInterval, imagesPerFlat, imagesPerDark, incOrient, flatsAtEnd):
    ntot = nPoints
    if imagesPerDark > 0:
        ntot += imagesPerDark
    nbatch_flat = 1 + nPoints // flatFieldInterval
    norient = nbatch_flat * (3 + 3)
    if flatsAtEnd and not (nPoints % flatFieldInterval == 0):
        nbatch_flat += 1
        norient += 3
    ntot += nbatch_flat * imagesPerFlat
    if incOrient:
        ntot += norient
    return ntot
"""
...
"""
def txm_tomoScan(inBeamPosition, outOfBeamPosition, exposureTime=0.1, start=0., angRange=180., nPoints=1500, flatFieldInterval=0,
              imagesPerDark=20, imagesPerFlat=20, min_i=-1., addNXEntry=True, autoAnalyse=True, flatFieldAngle=None, incOrient=True, txm_tomography_detector=None, additionalScannables=[], **kwargs):
    """
    Function to collect a tomogram
 	Arguments:
    inBeamPosition - position of X drive to move sample into the beam to take a projection
    ...
    startTm = datetime.datetime.now()
    nPoints = int(nPoints)
    _theta, step = dnp.linspace(start, start + angRange - float(angRange) / nPoints, num=nPoints, retstep=True)
    stop = _theta[-1]
    print "Args START"
    if not txm_tomography_detector is None:
        print "txm_tomography_detector = %s" %(txm_tomography_detector.getName()) 
...
#    print "darkFieldInterval = %i" %(darkFieldInterval)
    ...
    if txm_tomography_detector is None:
        print "txm_tomography_detector = None"
    else:
        print "txm_tomography_detector = %s" %(txm_tomography_detector.getName())
    ...
    if flatFieldAngle is None:
        ...
        msg += " with the first flat-field being taken at the scan's start angle of %.4f deg" %(start)
        print msg
    else:
...
#	        raise NameError("tomodet is not defined in Jython namespace")
        ...
        txm_tomography_theta=jns.txm_tomography_theta
        if txm_tomography_theta is None:
            raise NameError("txm_tomography_theta is not defined in Jython namespace")
        txm_tomography_shutter=jns.txm_tomography_shutter
        if txm_tomography_shutter is None:
            raise NameError("txm_tomography_shutter is not defined in Jython namespace")
        txm_tomography_translation=jns.txm_tomography_translation
        if txm_tomography_translation is None:
            raise NameError("txm_tomography_translation is not defined in Jython namespace")
        
        if txm_tomography_detector is None:
	        txm_tomography_detector=jns.txm_tomography_detector
        if txm_tomography_detector is None:
            raise NameError("txm_tomography_detector is not defined in Jython namespace")
...
#        meta_add( sample_stage)
        ...
        tomoScanDevice = make_tomoScanDevice(txm_tomography_theta, txm_tomography_shutter, 
                                             txm_tomography_translation, image_key, index, use_slits=True)

        numberSteps = ScannableUtils.getNumberSteps(txm_tomography_theta, start, stop, step)
        theta_points = []
...
#        shutterOpen=1
#        shutterClosed=0
        shutterOpen=txm_tomography_shutter.getPosition()
        shutterClosed=-0.1
        scan_points = []
        theta_pos = theta_points[0]
        print tomoScanDevice.getGroupMembers()
        #tomoScanDevice.tomography_shutter.moveTo(shutterClosed)
        txm_tomography_shutter.moveTo(shutterClosed)
        index=0
        ...
        if incOrient:
            for a in [0, 90, 180]:
                scan_points.append((a, shutterOpen, inBeamPosition, 3 + a//90, index ))
                index = index + 1     
...
#                index = index + 1        
        ...
        print scan_points
        print getTotNumScanPoints(nPoints=nPoints, flatFieldInterval=flatFieldInterval, imagesPerFlat=imagesPerFlat, imagesPerDark=imagesPerDark, incOrient=incOrient, flatsAtEnd=True)
        positionProvider = tomoScan_positions( start, stop, step, imagesPerDark, flatFieldInterval, imagesPerFlat, \
                                               inBeamPosition, outOfBeamPosition, scan_points ) 
        scan_args = [tomoScanDevice, positionProvider, tomography_time, tomography_beammonitor, txm_tomography_detector, exposureTime ]
        if min_i > 0.:
        ...
        if addNXEntry:
            addNXTomoSubentry(scanObject, txm_tomography_detector.name, txm_tomography_theta.name, **kwargs)
#        tomodet.stop()
        scanObject.runScan()
        ...
        #txm_tomography_shutter.moveTo( "Close")	
        #turn camera back on
#        tomodet.setupForAlignment()
        txm_tomography_translation.moveTo(inBeamPosition)
            
        return scanObject;
    except :
        exceptionType, exception, traceback = sys.exc_info()
        handle_messages.log(None, "Error in txm_tomoScan", exceptionType, exception, traceback, True)
    finally:

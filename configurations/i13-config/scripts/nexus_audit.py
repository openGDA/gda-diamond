...
from i13i_utilities import pwd, cfn#, getVisitRootPath, setSubdirectory, getSubdirectory
...
nxs_audit = NexusAudit("i13-2") # for i13 Imaging branch
...
def nxs_audit_tomo_step_scan(outdirpath=None, runScan=True, sleepdelta=1, niter=30):
    ...
    if runScan:
        ...
        LocalProperties.set("gda.data.scan.datawriter.datadir", "/dls/i13/data/2015/cm12165-3/tmp")  #TO-DO
        ...
        try:
            ...
            LocalProperties.set("gda.data.scan.datawriter.datadir", "/dls/i13/data/2015/cm12165-3/raw")  #TO-DO
    ...
    dir_src = "/dls/i13/data/2015/cm12165-3" + os.sep + (subd if runScan else "raw") + os.sep    #TO-DO    
    dir_dst = outdirpath
    if not dir_dst:
        dir_dst = nxs_audit.outdirpath[1]   # the one with trailing slash
    #dir_dst = "/dls/tmp/vxu94780/nexus_audit/i13-2/" # useful override for testing
...
def nxs_audit_tomo_fly_scan(outdirpath=None, runScan=True, sleepdelta=2, niter=60):
    """
    Desc:
    To run tomoFlyScan and then copy the resulting scan files to the Nexus Audit dir or any specified output dir.
        Note: Depends on the presence of the nxs_audit object (defined at the top of this file) 
    Arg(s):
    outdirpath - abs path to output dir to which scan files are to be copied
        Note: if not None, this outdirpath will be used; otherwise nxs_audit.outdirpath will be used instead
    ...
    if runScan:
        ...
        #addNXEntry=True    # tomoFlyScan on i13-2 does it every time
        autoAnalyse=False
        ...
        #      imagesPerDark=20, imagesPerFlat=20, min_i=-1., setupForAlignment=False, autoAnalyse=True):
        try:
            tomoFlyScan(ss1_X_dummy(), ss1_X_dummy(), expoTime, \
                     startAng, stopAng, stepAng, \
                     darkFieldInterval=0, flatFieldInterval=0, imagesPerDark=2, imagesPerFlat=2, \
                     min_i=-1., setupForAlignment=False, autoAnalyse=autoAnalyse)
            print "Finished running the scan!" 
        except:
            exceptionType, exception, traceback = sys.exc_info()
            handle_messages.log(None, "Exception in %s" %(thisfn), exceptionType, exception, traceback, False)

...
from scannables.id_energys.idd_switchable import idd_pos_neg_switcher,\
	idd_pos_neg_switchable
ix = SimpleScannable()
ix.name = "ix"
ix.moveTo(0)
...
class ScanPositionProviderFromFile(ScanPositionProvider):
    ...
    def load(self, filepath=None, offset=None, scale=None):
        if filepath != None:
            print("Setting file to %s" % filepath)
            self.filepath = filepath
        
        if offset != None:
            print("Setting offset to %s" % offset)
            self.offset=offset
        
        if scale != None:
            print("Setting scale to %f" % scale)
            self.scale = scale

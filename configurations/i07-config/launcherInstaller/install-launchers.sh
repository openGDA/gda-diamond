...
if [ ! -h "/dls/$1/software/gda" ]; then
	echo "This script requires a valid link '/dls/$1/software/gda' pointing to the GDA installation. "
	exit 1
fi
...
if [ ! -f "/etc/profile.d/beamline.sh" ]; then
	...
	sudo cp -f /dls/$1/software/gda/config/launcherInstaller/beamline.sh /etc/profile.d
...
sudo cp -f /dls/$1/software/gda/config/launcherInstaller/gda_launchers.sh /etc/X11/xinit/xinitrc.d/gda_launchers.sh
...
sudo cp -f /dls/$1/software/gda/config/Desktop/*.desktop /var/Desktop
sudo cp -f /dls/$1/software/gda/config/Desktop/*.menu /var/Desktop
sudo cp -f /dls/$1/software/gda/config/Desktop/*.directory /var/Desktop
sudo cp -f /dls/$1/software/gda/config/Desktop/.hidden /var/Desktop
...
echo
if [ ! -d "/dls/$1/etc" ]; then
	echo "Please log into $1-control to create directory /dls/$1/etc"
fi
# copy files
cp -f /dls/$1/software/gda/config/etc/${1}_profile.sh /dls/$1/etc
ls -l /dls/$1/software/gda/config/etc/${1}_profile.sh
echo "Please make sure /dls/$1/etc/${1}_profile.sh is EXECUTABLE."
echo
#### set up remote-start-up script ####
ls -l /dls/$1/software/gda/config/bin/remotestartupscript.sh
echo "Please make sure /dls/$1/software/gda/config/bin/remotestartupscript.sh is EXECUTABLE."
echo
#### set up remote start GDA key ###
if [ ! -r "/dls/$1/${1}ssh.key" ]; then
	cp -f /dls/$1/software/gda/config/${1}ssh.key /dls/$1/
fi
ls -l /dls/$1/${1}ssh.key
echo "Please make sure /dls/$1/${1}ssh.key file is owned by 'gda:gda'."
echo 

...
#				self.owner.status = FastScanControlClass.MOTOR_STATUS_READY;
	...
	def __init__(self, name, scannableMotor):
		self.name = name;
		...
		self.scannableMotor=scannableMotor;
		self.motor = scannableMotor.getMotor();
		self.backupMotor();
	...
	def getRealPosition(self):
		return self.scannableMotor.getPosition();
...
#		self.stopPosition = self.stopPosition;
	...
	def build(self):
		self.update()
		self.scannableMotor.moveTo( self.getStartPosition() );
...
#		testMotor1.asynchronousMoveTo(self.getEndPosition());
		self.scannableMotor.asynchronousMoveTo(self.getEndPosition());

	def abortScan(self):
		self.restoreMotor();


class EpicsPositionComparationDeviceClass(FastScanControlClass):
	OUTPUT_MODE = range(3);
	OUTPUT_MODE_STRING = ['Off', 'On', 'Auto'];
	
	def __init__(self, name, scannableMotor, rootPV):
		FastScanControlClass.__init__(self, name, scannableMotor);
	...
	def __del__(self):
		...
		self.cleanChannel(self.chStep);
		self.cleanChannel(self.chPulse);
	
	"""
	rootPV:      BL07I-MO-DIFF-01:OMEGA:POSCOMP
	Output Mode: BL07I-MO-DIFF-01:OMEGA:POSCOMP:CTRL
	
	Range Start: BL07I-MO-DIFF-01:OMEGA:POSCOMP:START
	Range Stop : BL07I-MO-DIFF-01:OMEGA:POSCOMP:STOP
	
	Pulse Step:  BL07I-MO-DIFF-01chMode:OMEGA:POSCOMP:STEP
	Pulse Width: BL07I-MO-DIFF-01:OMEGA:POSCOMP:PULSE
	"""
	def setupEpics(self, rootPV):
		self.chMode =CAClient(rootPV + ":CTRL"); self.configChannel(self.chMode);
		...
		self.chStep=CAClient(rootPV + ":STEP"); self.configChannel(self.chStep);
		self.chPulse=CAClient(rootPV + ":PULSE"); self.configChannel(self.chPulse);
...
#	Epics level operation:
	def setEpicsMode(self, newMode):
		if newMode in EpicsPositionComparationDeviceClass.OUTPUT_MODE:
			nm=newMode;
		elif newMode in EpicsPositionComparationDeviceClass.OUTPUT_MODE_STRING:
			nm=EpicsPositionComparationDeviceClass.OUTPUT_MODE_STRING.index(newMode);
		else:
			nm = 0;
			
		self.chMode.caput(nm);
			
		
	def setEpicsRange(self):
		self.chStart.caput(self.startPosition)
		self.chStop.caput(self.stopPosition)
		
	def setEpicsPulse(self):
		s=self.getStep();
		self.chStep.caput(-1.0*s);
		self.chPulse.caput(-1.0*s/2.0)
...
class FastMotionDeviceClass(ScannableMotionBase):
	...
	def __init__(self, name, fastScanController, fastScanDetector):
		self.setName(name);
		name1=fastScanController.scannableMotor.getName();
		name2=fastScanController.scannableMotor.getName()+"RBV";
...
#		print "At Scan Start"
		...
		print "...Start scan"
		print "---> Debug: Current Time: " + ctime();
		print "---> Debug: Current Position: " + str(self.fastController.scannableMotor.getPosition());
...
#		self.setInternalTrigger(True);
	...
	Trigger Selection: BL07I-EA-DET-01:MCA-01:ChannelAdvance
	"""
	def setupEpics(self, rootPV):
		...
		#Epics PV for the trigger signal selection
		self.chTriggerSelection=CAClient(rootPV + ":ChannelAdvance");  self.configChannel(self.chTriggerSelection);
...
#	Epics level operation:
	...
	def setInternalTrigger(self, bInternal=True):
		if bInternal:
			self.chTriggerSelection.caput(0);
		else:
			self.chTriggerSelection.caput(1);
...
pcPV = "BL07I-MO-DIFF-01:OMEGA:POSCOMP"
mcaPV= "BL07I-EA-DET-01:MCA-01";
numberOfMCA=1;

#exec([fastController, fastData, fastMotion]=[None, None, None]);

#A function to run the fast scan
def cvscan(scannableMotor, startPosition, stopPosition,scanTime, pointTime):
...
#	fastData.setInternalTrigger(True);
	fastData.setInternalTrigger(False);

...
pil3 = ADPilatusPseudoDeviceClass("pil3", "Pilatus 3", "pilatus3");
pil3.setAlive(False);
pil3.addShutter('fs');
...
#For the RCP GUI
pil3sum = AnalyserDetectorClass("pil3sum", pil3, [SumProcessor()], panelName="Pilatus 3", iFileLoader=PilatusTiffLoader);
pil3sum.setAlive(True);
pil3sum.readoutNexus = True

pil3stats = AnalyserDetectorClass("pil3stats", pil3, [MinMaxSumMeanDeviationProcessor()], panelName="Pilatus 3", iFileLoader=PilatusTiffLoader);
pil3stats.setAlive(True);
pil3stats.readoutNexus = True

pil3fit = AnalyserDetectorClass("pil3fit", pil3, [TwodGaussianPeak()], panelName="Pilatus 3", iFileLoader=PilatusTiffLoader);
pil3fit.setAlive(True);
pil3fit.readoutNexus = True

pil3roi = AnalyserWithRectangularROIClass("pil3roi", pil3, [MinMaxSumMeanDeviationProcessor()], panelName="Pilatus 3", iFileLoader=PilatusTiffLoader);
...
pil3roisum = AnalyserWithRectangularROIClass("pil3roisum", pil3, [SumProcessor()], panelName="Pilatus 3", iFileLoader=PilatusTiffLoader);

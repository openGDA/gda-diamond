...
class ADPilatusPseudoDeviceClass(DetectorBase, DetectorSnapper, ShutterDeviceClass, MetadataConsumerClass, IObserver):
...
# Detector Implementation
	...
	def prepareForCollection(self):
		...
		self.checkPath();
		self.detector.adBase.setNumImages(self.num_images)
...
#        print "Debug: "+  sys._getframe().f_code.co_name +" is called";
	...
	def atScanEnd(self):
		self.resetFilePrefix()

	def atScanStart(self):
		if self.cachedFilePrefix is None:
			scanNumber = InterfaceProvider.getCurrentScanInformationHolder().getCurrentScanInformation().getScanNumber()
			self.cachedFilePrefix = self.filePrefix
			self.setFile(newFilePrefix=self.cachedFilePrefix + '_' + str(scanNumber) + '_')
...
#		print "Debug: "+  sys._getframe().f_code.co_name +" is called";
	...
	def toString(self):
		#self.getPosition();
		try:
			lastFile = "Latest image file: " + self.getFullFileName();
			return lastFile
		except:
			return "Pilatus: Could not get lastest image file"

	def resetFilePrefix(self):
		if self.cachedFilePrefix:
			self.setFile(newFilePrefix=self.cachedFilePrefix)
			self.cachedFilePrefix = None

...
pro rebin_delta_tau,path,out_path,file_list,user_step_size,argument,user_comment
  ;
  ;=================================================================
  ;THIS PRO REBINS DELTA-TAU REBINNED DATA EITHER BATCH OR SUMMATION
  ;=================================================================
  ;
  catch,error_status
  if error_status ne 0 then begin
     print,"***-FAILED IN rebin_delta-tau"
     catch,/cancel
     return
  endif
  ;
  ...
  for i=0,n_elements(file_list)-1 do begin
     ;
     print,"-----opening ",path+file_list(i)
     openr,1,path+file_list(i)+".dat"
     line=""
     j=0
     ...
     header=header(0:j-1)
     j=j+1L
     readf,1,line   ;<-----read column headers line
     ;
     print,"-----reading data"
     j=0L
;     while ~EOF(1) do begin
     while j lt 150000 do begin
        readf,1,a,b,c
        x(j)=a
        y(j)=y(j)+b
        e(j)=e(J)+c*c
     ...
     case 1 of
       ...
       (argument eq "-B")):begin
                             ;batch rebin  
                             print,"-----calling larger rebin"
                             larger_rebin,x,y,e,user_step_size
                             summed_file_name=file_list(i)+"_"
                             print,"-----summed file name ",summed_file_name
                             print,"-----calling output_rebinned "
                             output_rebined_file,summed_file_name,out_path,header,x,y,sqrt(e),user_comment,x(0),max(x),argument
                             x=dblarr(150000)*0.0
                             y=dblarr(150000)*0.0
                             e=dblarr(150000)*0.0
                           end
       else:begin
              run_number=strmid(file_list(i),0,strpos(file_list(i),"-mac"))
              print,"+++++run number ",run_number
              ;summed_file_name=summed_file_name+file_list(i)+"_"
              summed_file_name=summed_file_name+run_number+"_"
              print,"-----summed file name  ",summed_file_name
            end
     endcase
     ;
  ...
  case 1 of
    ...
    (argument eq "-A")): begin
                           e=sqrt(e)
                           if user_step_size gt 0.001 then begin
                               print,"-----calling larger_rebin"
                               larger_rebin,x,y,e,user_step_size
                           endif
                           print,"-----calling output_rebinned"
                           print,"--------summed file name ",summed_file_name
                           print,"--------out put path ", out_path
...
pro rebin_idl_delta_tau,arg1_string,arg2_string,arg3_string,arg4_string,arg5_string,return_string
  ;
  catch,error_status
  if error_status ne 0 then begin
     rebin_idl

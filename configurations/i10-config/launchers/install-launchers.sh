...
if [ ! -h "/$2/$1/software/gda" ]; then
	echo "This script requires a valid link '/$2/$1/software/gda' pointing to the GDA installation. "
	exit 1
fi
...
if [ ! -f "/etc/profile.d/beamline.sh" ]; then
	...
	sudo cp -f /$2s/$1/software/gda/config/launchers/beamline.sh /etc/profile.d
...
sudo cp -f /$2/$1/software/gda/config/launchers/gda_launchers.sh /etc/X11/xinit/xinitrc.d/gda_launchers.sh
...
sudo cp -f /$2/$1/software/gda/config/Desktop/*.desktop /var/Desktop
sudo cp -f /$2/$1/software/gda/config/Desktop/*.menu /var/Desktop
sudo cp -f /$2/$1/software/gda/config/Desktop/*.directory /var/Desktop
sudo cp -f /$2/$1/software/gda/config/Desktop/.hidden /var/Desktop
...
sudo ln -sf /var/Desktop/gda-diffcalc-guide.desktop /usr/share/applications/gda-diffcalc-guide.desktop
...
echo
if [ ! -d "/$2/$1/etc" ]; then
	echo "Please log into $1-control to create directory /$2/$1/etc"
fi
# copy files
cp -f /$2/$1/software/gda/config/etc/${1}_profile.sh /$2/$1/etc
ls -l /$2/$1/software/gda/config/etc/${1}_profile.sh
echo "Please make sure /$2/$1/etc/${1}_profile.sh is EXECUTABLE."
echo
#### set up remote-start-up script ####
ls -l /$2/$1/software/gda/config/bin/remotestartupscript.sh
echo "Please make sure /$2/$1/software/gda/config/bin/remotestartupscript.sh is EXECUTABLE."
echo
#### set up remote start GDA key ###
if [ ! -r "/$2/$1/${1}ssh.key" ]; then
	cp -f /$2/$1/software/gda/config/${1}ssh.key /$2/$1/
fi
ls -l /$2/$1/${1}ssh.key
echo "Please make sure /$2/$1/${1}ssh.key file is owned by 'gda:gda'."
echo 

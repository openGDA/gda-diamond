...
class DetectorAnalyserWithMovingRectangularROIClass(DetectorAnalyserWithRectangularROIClass):
	
	def __init__(self, name, detector, processors=[], panelName="Area Detector", iFileLoader=None):

		DetectorAnalyserWithRectangularROIClass.__init__(self, name, detector, processors, panelName, iFileLoader);
...
#			[x,y,w,h,angle]=self.getRoiParameters(roi);
	...
	def getPosition(self):
		...
		result=DetectorAnalyserWithRectangularROIClass.getPosition(self);
		return result;
...
ppr1 = DetectorAnalyserWithMovingRectangularROIClass("ppr1", pil1, [MinMaxSumMeanDeviationProcessor()], panelName="Area Detector", iFileLoader=PilatusTiffLoader);

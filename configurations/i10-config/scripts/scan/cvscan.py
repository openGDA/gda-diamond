...
class TrajectoryControllerHelper(ScanListener):
    ...
    def update(self, scanObject):
        ...
        if self.energy.getName() == "egy_g" or self.energy.getName() == "egy":
            for each in self.wfs:
                each.setHardwareTriggerProvider(energycontroller) # switch to ContinuousPgmGratingIDGapEnergyMoveController @UndefinedVariable 

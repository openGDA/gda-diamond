...
import unittest
import re
from ccdScripts import ccdScan
...
class Run:
	...
	def getExposureTime(self):
		return self.values["dexposuretimeinsec"]
...
class TestPosition(unittest.TestCase):
	...
	def testRunListLoader(self):
		runList = RunListLoader().getRunList(self.controller,"C:/Data/GDA/garnet/garnet.run")
#		runList.dump()
		runList.dumpRuns()
#		for irun in range(0,len(self.runs)):
#			run = self.runs[irun]
#			axis = run.getScantype()=="Phi" ? phi, omege
#			ccdScan(run.getExposureTime(), runList.info.getExperimentName(),

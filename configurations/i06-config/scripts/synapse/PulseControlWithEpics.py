...
def generatePulseAndMeasureFromKeithley(pulse_level, pulse_width, time_delay, number_of_pulses, nplc, timeout = 1.0):
    keithley.reset()
    keithley.sourceFunction('CURR')
    keithley.senseFunction('VOLT')
    ...
    print(SLEEP_FOR_FORAMT % ((pulse_width + time_delay) * number_of_pulses * 3 + 0.1))
    sleep((pulse_width + time_delay) * number_of_pulses * 3 + 0.1)
    data = keithley.readTraceData(number_of_pulses, timeout)
    return data

        
def pulseAndMeas(timeout = 1.0, *args):
    ...
    if len(args) == 7:
        ...
        data = generatePulseAndMeasureFromKeithley(args[2], args[3], args[4] / 2, args[5], args[6], timeout)
        print(data)
        synapse_setall.rawAsynchronousMoveTo("STV")
        ...
        data = generatePulseAndMeasureFromKeithley(args[4], args[5], args[6] / 2, args[7], args[8], timeout)
        print(data)
        synapse_setall.rawAsynchronousMoveTo("STV")
...
def measureResistance(count, timeout = 1.0):
    keithley.reset()
    keithley.sourceFunction('CURR')
    keithley.senseFunction('RES')
    keithley.senseAutoRange('RES', 'ON')
    keithley.senseResistanceCompensated('ON')
    resistance = keithley.readResistance(count, timeout)
    return resistance

        
def measureVoltage(val, comp_voltage, auto_range, range_value, state, nplc, timeout = 1.0):
    keithley.reset()
    keithley.sourceFunction('CURR')
...
def probeRes(timeout = 1.0, *args):
    ...
    if len(args) == 7:
        ...
        voltage = measureVoltage(args[2], args[3], args[4], args[5], 'OFF', args[6], timeout)
        print("2-point")
        ...
        voltage = measureVoltage(args[4], args[5], args[6], args[7], 'ON', args[8], timeout)
        print("4-point")

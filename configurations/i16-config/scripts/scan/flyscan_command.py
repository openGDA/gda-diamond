...
To perform a fly scan of scannable 'tx' over range (start, stop, step) and measure detector 'det' at each step of 'tx'
...
To perform a fly scan as the inner most scan of a nested scan
    e.g. perform a scan of 'ty' over range (ystart, ystop, ystep) and at each point of 'ty' perform the above flyscan of 'tx'
...
class FlyScannable(ScannableBase):
    ...
    :usage:
    #set the speed of flying motor first before using following
...
def flyscan(*args):
    ...
    newargs=[]
    newargs = parse_flyscan_scannable_arguments(args, newargs)  
...
def flyscancn(*args):
    ...
    newargs=[]
    newargs, flyscannablewraper, current_position = parse_flyscancn_scannable_arguments(args, newargs)
  ...
  See also scan."""
    ...
    def parseArgsIntoArgStruct(self, args):
        if list(args).count(flyscan) == 1:
            ...
            return ConcurrentScanWrapper.parseArgsIntoArgStruct(self, parse_flyscan_scannable_arguments(fly_args[1:], args_before))

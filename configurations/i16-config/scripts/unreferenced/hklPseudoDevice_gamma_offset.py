...
class hklPseudoDevice_gamma_offset(ScannableMotionBase):
   ...
   def __init__(self,name,euler,tth,rs,cal,EDi,delta,gam,az,gam_offsetpd,flag):
      ...
      self.cal= cal
      self.EDi = EDi
      self.setInputNames(['h', 'k','l','gam'])
      self.delta = delta
      self.gam = gam
      self.setName("hkl_gam")
      self.setOutputFormat(["%4.4f","%4.4f","%4.4f","%4.4f"])
      self.az=az
      self.gam_offset = gam_offsetpd
      self.flag =flag
      self.statichelp= """This modified hkl pseudodevices takes hkl and an offset in gamma as arguments.
      It positions hkl to the required hkl + the offset in gamma. It returns, depending on the flag argument (1 or 2)
      either the hkl initially required and the offset, or the real position in hkl and the offset.
      It needs to be linked to an instance of the offset class containing the offset in gam.
      """
#      if help is not None:
...
#         self.__doc__+='\nHelp specific to '+self.name+':\n'+self.statichelp
   ...
   def asynchronousMoveTo(self,new_position):
      self.gam_offset(new_position[3])
      ...
      euler_positions = [angles.Phi,angles.Chi,angles.Eta,angles.Mu,angles.Delta,angles.Gamma+new_position[3]]
...
#         panicStop()
	...
	# TODO: Must also check delta and gamma!
   def isBusy(self):

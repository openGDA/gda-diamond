...
class szCryoCompensation(PseudoDevice):
    ...
    def calibrate(self,device,channel):
        ...
        Temp=eval(self.channel+str('()'))
        self.szoffset = self.poly(Temp,self.cryopoly)-sz()
        print('Sample offset = '+str(self.szoffset))

    def asynchronousMoveTo(self,value):
        szcomp = self.poly(eval(self.channel+str('()')),self.cryopoly)-self.szoffset - sz() + value
        sz.asynchronousMoveTo(szcomp + sz())

    def getPosition(self):
        try:
            szcom = self.poly(eval(self.channel+str('()')),self.cryopoly)-self.szoffset -sz()
            return szcom 
        except:

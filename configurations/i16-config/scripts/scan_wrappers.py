...
import time
from gda.jython.commands.ScannableCommands import createScanPlotSettings, configureScanPipelineParameters
class ScanWrapperClass:
	...
	def __call__(self,*params):
		global CurrentScanObject, SRSWriteAtFileCreation, lcen, rcen
...
#		print old_paramlist
		...
		self.scan_string=self.create_scan_command_string(self.newparams)
		#SRSWriteAtFileCreation="\ntestdata1='letsagetabitarockin'\n"
		SRSWriteAtFileCreation="\n"+self.scan_string+"\n"
...
#		print 'new params:', self.newparams
		...
		try:
			####delete from here of problem####
			lcen=add(self.paramlist[0][0],mult(-1.0,add(mult(0.5,self.paramlist[0][1]), mult(-0.5,self.paramlist[0][0]))))#left centre
			rcen=add(self.paramlist[0][1],mult(1.0,add(mult(0.5,self.paramlist[0][1]), mult(-0.5,self.paramlist[0][0]))))#left centr
			#### delete to here if problem
			self.ScanObject=ConcurrentScan(self.newparams)
			configureScanPipelineParameters(self.ScanObject)
			self.ScanObject.setScanPlotSettings(createScanPlotSettings(self.ScanObject))
			self.ScanObject.runScan()		#do scan
	...
	def create_scan_command_string(self, scanparams):
		outstr="scancommand='scan"
		for tok in scanparams:
			try:
				outstr+=' '+tok.name
			except:
				outstr+=' '+str(tok)
		return outstr+"'"	#add closing quote
...
#		print 'full_name_list', full_name_list
	...
	def get_start_positions(self,pd):
		try:
			...
			pdpos=pd()			#new fix - in case PD returns a scalar and can't be sliced
			#pdpos=pd[:]
		return pdpos
			
		#temp version as PD slicing stopped working
		#try:
		#	pdpos=pd()[:]			#list converts to list. remove when bug fixed
		#	if len(pdpos)==1:		#this line not needed if bug fixed. 
		#		pdpos=pdpos[0]		#this line not needed if bug fixed
		#except:
		#pdpos=pd()			#must be a scalar if exception raised
		#return pdpos

	def get_scan_stats(self):
		#get cen value assuming first PD in scan list is x and last collumn of the last PD is y
		print "In module scan_wrappers.py Sleeping while file appears for 1s in scan_wrappers.get_scan_stats"
		sleep(1)
		...
		try:
			...
			maxval=FindScanPeak((list(pdy.getInputNames())+list(pdy.getExtraNames()))[-1]);	#use last collumn of pdy
#		print 'max=',maxval
#		print pdx.getInputNames()
	...
	def get_scan_plot_strings(self):
		#similar to get_scan_stats but returns x and y plot strings
		pdx=self.devlist[0];
		pdy=self.devlist[-1];
		try:
			ystring=det;					#collumn from global det
		except:
			ystring=''
		if ystring=='':
			ystring=(list(pdy.getInputNames())+list(pdy.getExtraNames()))[-1];	#use last collumn of pdy
		xtring=pdx.getInputNames()[0]				#assume first collumn for now...
...
class ScanWrapperReturnToStartClass(ScanWrapperClass):
	...
	counts are taken from last column of last device entered in the scan line, or use det=outputname (e.g. det='APD') to overide
	maxpos is peak value
	lcen is the centre of the scan minus the scan range, i.e. a centred scan about this point gives a scan that connects to the current one
	rcen is the same but on the upper side
	'''
...
class ScanWrapperScanClass(ScanWrapperClass):	
	'''
	This does a normal GDA scan within a wrapper to carry out extra functionality
	scan dev1 start end step dev2 start end step dev3 start dev4 
	See GDA manual
	go command moves first PD from scan to value, e.g. 'go maxpos'	
	counts are taken from last column of last device entered in the scan line, or use det=outputname (e.g. det='APD') to overide
	lcen is the centre of the scan minus the scan range, i.e. a centred scan about this point gives a scan that connects to the current one
	...
	def __init__(self):
		global det, CurrentScanObject						#used for data collumn for peak value if this collumn is present in scan
		self.name='scan'							#name of the scan type

	def convert_scan_params(self, devlist,paramlist):				#no change to scan parameters for normal scan
		return	[devlist, paramlist]	
...
class ScanWrapperDscanReturnToStartClass(ScanWrapperReturnToStartClass):	
	...
	counts are taken from last column of last device entered in the scan line, or use det=outputname (e.g. det='APD') to overide
	lcen is the centre of the scan minus the scan range, i.e. a centred scan about this point gives a scan that connects to the current one
	rcen is the same but on the upper sid
	'''
...
oldscan = scan
exec('scan=ScanWrapperScanClass()')
alias dscan
alias lup
alias ascan
alias scancn

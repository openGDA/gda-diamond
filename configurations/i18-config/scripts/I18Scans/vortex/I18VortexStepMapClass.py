...
class I18VortexStepMapClass(ScriptBase):
	...
	def stepmapscan(self,xstart,xend,xstep,ystart,yend,ystep,collectionTime):
		...
		#for i in range(self.noOfROI):
		#	self.fluData.append(DataSet([nx,ny]))
		self.checkForInterrupt()
		self.vortex.stop()
		for i in range(ny):
			#
			...
			for j in range(nx):
				...
				self.checkForInterrupt()
				#self.writeToRGB(i,j,ionChambers[0],ionChambers[1],ionChambers[2],vortexdata.tolist())
	...
	def writeToRGB(self,nx,ny,i0,it,id,flu):
		...
		##converting from list to space separated string
		#mystr=""
		#for k in range(len(flu)):
		#	mystr=mystr+str(flu[k])
		#	if(k<(len(flu)-1)):
		#		mystr=mystr+" "
		#print >> fid,nx,ny,str(i0).strip(),str(it).strip(),str(id).strip(),mystr

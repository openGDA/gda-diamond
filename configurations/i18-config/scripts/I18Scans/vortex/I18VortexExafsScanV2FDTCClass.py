...
class I18VortexExafsScanClass(ScriptBase):
    def __init__(self):
        ...
        self.detectorNamesVector.add(finder.find("xmapMca")) 
...
#
    ...
    def writeSummary(self,currentpos,data,vortexdata,vortexdatasum,vortexDTC,collectionTime):
        ...
        print >>fid,currentpos,comboDCM_eV.getPosition(),collectionTime,data[0],data[1],data[2],str(vortexdata2[0:]).strip('[]').replace(',',''),vortexdatasum[0],str(vortexDTC).strip('[]').replace(',','')
        fid.close()
        ...
        print filename
        sdp.setCurrentFilename(filename)
    ...
    def timeForK(self,k,start,end,kWeighting,kEndTime,kStartTime):
        ...
        # round to nearest 0.1 milliseconds as this is all the ion chambers can collect in
        time=Math.round(time/0.1) * 0.1
        return time
    #
    #

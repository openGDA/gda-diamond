...
class VortexDTCParams:
	def __init__(self):
		...
		for i in range(4):
			...
			self.channels.append(CAClient(basestring+"REALTIME"))
			self.channels[-1].configure()
		for i in range(4):
			basestring=xmapstring+str(i)+":DATA:"
			self.channels.append(CAClient(basestring+"TRIG_LIVETIME"))			
			self.channels[-1].configure()
		for i in range(4):
			basestring=xmapstring+str(i)+":DATA:"
			self.channels.append(CAClient(basestring+"ENERGY_LIVETIME"))			
			self.channels[-1].configure()
		for i in range(4):
			basestring=xmapstring+str(i)+":DATA:"

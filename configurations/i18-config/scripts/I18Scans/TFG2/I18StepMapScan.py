...
class I18StepMapClass(ScriptBase):
    ...
    def writeSummaryAtPoint2(self,currentx,currenty,xindex,yindex,collectionTime,files):
        ionChamberData=counterTimer01.readFrame(xindex)
        fid = open(self.datafilename,'a')
        ...
        line=str(currentx)+"\t"+str(currenty)+"\t"+str(collectionTime)+"\t"+str(ionChamberData[0])+"\t"+str(ionChamberData[1])+"\t"+str(ionChamberData[2])+"\t"+str(files[0])
        print >> fid,line
        fid.close()
        print currentx,currenty,collectionTime,ionChamberData[0],ionChamberData[1],ionChamberData[2],files[0]
        # SDP Stuff
        # SDP Stuff

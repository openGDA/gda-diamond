...
class I18TransmissionExafsScanClass(ScriptBase):
    ...
    def anglescan(self,start,end,step,collectionTime):
        ...
        for i in range(npoints):
            ...
            self.checkForInterrupt()
            print currentpos,comboDCM_eV.getPosition(),collectionTime,mydata[0],mydata[1],mydata[2]
            self.writeSummary(currentpos,mydata,collectionTime)
            # Move the mono
    ...
    def kscan(self,start,end,step,kStartTime,kEndTime,kWeighting, edgeEnergy, twoD):
        ...
        for i in range(npoints):
            ...
            mydata=self.ionchambers.getPosition()
            self.checkForInterrupt()
            print mdegPosition,comboDCM_eV.getPosition(),mydata[0],mydata[1],mydata[2]
            self.checkForInterrupt()
    ...
    def writeSummary(self,currentpos,data,collectionTime):
        ...
        #sdp = ScanDataPoint("Exafs TransScan",self.scannableNamesVector,self.detectorNamesVector,None,None,None,None,positionVector,detectorVector,None,"Panel Name","I18 Custom SDP","Header String",self.datafilename,0)
        sdp = ScanDataPoint()
        #sdp.setUniqueName("Exafs TransScan")
        sdp.setUniqueName(str(self.fileno))
        sdp.setScanIdentifier("Exafs TransScan")
        ...
        sdp.addDetectorData(newdata,["%5.2g","%5.2g","%5.2g"])

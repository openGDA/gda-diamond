...
class I18ContinuousMapClass(ScriptBase):
		...
		try:
		   ...
		   # if not lock:
			...
			for i in range(nysteps):
				...
				for x in xpositions:
					realx.append(x)
				self.writeSummary(ymoveto, i, realx, actualPulses, totalPoints)
  ...
  #====================================================
	...
	def writeSummary(self, currenty, yindex, xpositions, nxpoints, totalPoints):
		...
		for i in range(nxpoints):
			...
			val = xpositions[i]
			print 'ic data again', i, ionchamberData, self.mcaList[mcafileIndex]
			##tfg read in 10ns blocks , converting to milli seconds
			pointTime = ionchamberData[0] * 10e-06
			line = str(val) + "\t" + str(currenty) + "\t" + str(pointTime) + "\t" + str(ionchamberData[1]) + "\t" + str(ionchamberData[2]) + "\t" + str(ionchamberData[3]) + "\t" + str(self.mcaList[mcafileIndex])
			print >> fid, line
			
			print (val), currenty, pointTime, ionchamberData[1], ionchamberData[2], ionchamberData[3], self.mcaList[mcafileIndex]
			# SDP Stuff
			detectorVector = Vector()
			detectorVector.add([ionchamberData[1], ionchamberData[2], ionchamberData[3]])
			detectorVector.add(self.mcaList[mcafileIndex])

...
class I18ExafsScanClass(ScriptBase):
    def __init__(self,detectorList=None):
        ...
        # Sample position at start of the scan...this is your reference position and
        # the stage tracks the beam about this position
        self.referenceX=MicroFocusSampleX.getPosition()
        self.referenceY=MicroFocusSampleY.getPosition()
        #self.setSampleMovementLookupTable()
        self.setSampleMovementLookupTable("/dls/i18/tmp/lookuptables/cu_seso_lookup.txt")
       ...
       # self.timeList=[]
        for i in range(npoints):
           ...
           # print 'check topup'
            ...
            self.checkForAngleInterrupt((i-1))
            #sleep(0.5)
            self.updateStagePosition(currentpos)
            ...
            if(i>=1):
                ...
                totalw=self.writeSummaryAtPoint(i-1,oldpos,collectionTime,npoints)
            ...
            if(i==npoints-1):
                ...
                totalw=self.writeSummaryAtPoint(i,currentpos,collectionTime,npoints)
    ...
    def kscan(self,start,end,step,kStartTime,kEndTime,kWeighting, edgeEnergy, twoD):
        ...
        for i in range(npoints):
          ...
          #  starttime=time.time()
            ...
            self.checkForKScanInterrupt((i-1))
#            sleep(0.5)
            self.updateStagePosition(mdegPosition)
            self.checkForKScanInterrupt((i-1))
            if(i==0):
                 self.das.sendCommand("tfg start")
            ...
            if(i>=1):
                ...
                totalw=self.writeSummaryAtPoint(i-1,oldpos,oldsecTime,npoints)
            ...
            if(i==npoints-1):
                ...
                ##get the mca data for the previous point
                self.writeDetectorFileAtPoint(i)
                totalw=self.writeSummaryAtPoint(i,mdegPosition,secTime,npoints)
    ...
    #==================================================
    def writeSummaryAtPoint(self,point_index,position,collectionTime,totalPoints):
        ...
        ##self.ionchamberData[point_index].append(0)
        ...
        sdp.setScanIdentifier("Exafs FluAngleScan")
        ...
        sdp.addDetectorData(self.ionchamberData[point_index],["%5.2g","%5.2g","%5.2g"])
        sdp.addDetectorData(windowedData,["%5.2g","%5.2g","%5.2g","%5.2g","%5.2g","%5.2g","%5.2g","%5.2g","%5.2g"] )
        sdp.setCurrentPointNumber(point_index)
        sdp.setNumberOfPoints(totalPoints - 1)
...
#==============================================================
    ...
    def read_movement_lookuptable(self):
        ...
        for i in range(len(AA)-1):
            a = AA[i].split()
            self.move_braggset.append(float(a[0]))
            self.move_xmov.append(float(a[1]))
            self.move_ymov.append(float(a[2]))
            #print self.move_braggset[-1],self.move_xmov[-1],self.move_ymov[-1]
    ...
    def setSampleMovementLookupTable(self,filename):
        print "using movement table :",filename
    ...
    def updateStagePosition(self,bragg):
        ...
        xmove=self.lookup_x_move(bragg)
        #print self.referenceY,ymove,self.move_ymov[0],self.referenceX,xmove,self.move_xmov[0]
        #print self.referenceY+((ymove-self.move_ymov[0])/1000.0),self.referenceX+((xmove-self.move_xmov[0])/1000.0)
        newxpos =self.referenceX+((xmove-self.move_xmov[0])/1000.0)
        newypos =self.referenceY+((ymove-self.move_ymov[0])/1000.0)
        if(fabs(newypos-MicroFocusSampleY.getPosition())>0.0001 or fabs(newxpos-MicroFocusSampleX.getPosition())>0.0001):
                  print "updating position to:",newxpos,newypos
	        pos MicroFocusSampleX newxpos
	        pos MicroFocusSampleY newypos

...
class I18ContinuousMapClass(ScriptBase):
	...
	def rastermapscan(self, startx, stopx, nxsteps, time, starty, stopy, nysteps):
		...
		try:
			...
			self.ionchambers.setCollectionTime(10000.0)
			   ...
			   # else:
				...
				for section in scanSections:
					...
					for j in range(len(self.windowName)):
						self.windowArrays.append(self.initArray1D(section[0])) 
						self.checkForInterrupt()
					...
					MicroFocusSampleX.moveTo(section[1] - 5 * stepx)
					self.checkForInterrupt()
					...
					#
					print 'move to end', section[2] + 5 * stepx
					MicroFocusSampleX.asynchronousMoveTo(section[2] + 5 * stepx)
					self.checkForInterrupt()
					...
					self.ionchamberData = self.ionchambers.collectData()
					self.checkForInterrupt()
					...
					self.xmotor.setSpeed(1.0)
					self.checkForInterrupt()
					MicroFocusSampleX.asynchronousMoveTo(section[1] + 5 * stepx)
					self.checkForInterrupt()
					#
					# process output
					#
			...
			handle_messages.log( None,"Error calling raster map Scan. ", type, exception , traceback, True)
			checkForInterrupt()
	...
	def appendToRGB(self, xindex, yindex, times, totalion, windowsums):
		...
		mystr = str(times) + " " + str(self.ionchamberData[0]) + " " + str(self.ionchamberData[1]) + " " + str(self.ionchamberData[2]) + " "
	...
	def writeoutDetectorDataRow(self, startx, stopx, stepx, currenty, yindex, noofxpoints, nameoffset):
		...
		while(currentpoint != noofxpoints + 1):
			...
			#print 'Reading and windowing:',files[0]
			#frameData=Xspress2Utilities.interpretDataFile(files[0],0)
	...
	def writeDetectorPointFile(self, yindex, xindex, nameoffset):
		...
		detfile = self.xspress.writeDummyFile()
		scafile = self.xspress.writeDummyScalerFile()
	...
	def readScalarDataNoRetry(self, point):
		...
		scalarString = self.readScalerfromMemory()
		if scalarString == "" :
			time.sleep(1)
			scalarString = self.readScalerfromMemory()
		try:
			...
			handle_messages.log(None, "Error in readScalarDataNoRetry - scalarString = " + `scalarString` , type, exception, None, True)
	...
	def writeSummary(self, currenty, yindex, startx, stepx, nx, offset):
		...
		for i in range(nx):
			val = (startx + (i * stepx))
			line = str(val) + "\t" + str(currenty) + "\t" + str(self.pointTime[i]) + "\t" + str(self.ionchamberData[0]) + "\t" + str(self.ionchamberData[1]) + "\t" + str(self.ionchamberData[2]) + "\t" + str(self.mcaList[i])
			print >> fid, line
			print (startx + (i * stepx)), currenty, self.pointTime[i], self.ionchamberData[0], self.ionchamberData[1], self.ionchamberData[2], self.mcaList[i]
			# SDP Stuff
			detectorVector = Vector()
			detectorVector.add(self.ionchamberData)
			...
			for j in range(len(self.windowName)):
				...
				print "WINDOWED DATA 1!"+ `windowedData`
				#print "WINDOWED DATA 1!"+`windowedData1`
				for k in range(9):
					totalw = totalw + windowedData[k]
				#
				# now set the data arrays with these values

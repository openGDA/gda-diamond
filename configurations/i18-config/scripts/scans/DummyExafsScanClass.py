import gda.gui.exafs.i18.Converter
...
class DummyExafsScanClass(ScriptBase):
    def __init__(self):
        ...
        print "inside the init methiod"
        self.scannableNamesVector=[]
        self.scannableNamesVector.append(dcm_mono)
        self.detectorNamesVector=[]
        self.detectorNamesVector.append(counterTimer01)
        self.detectorNamesVector.append(counterTimer02)
        # 
    ...
    def startScan(self):
        #
          ...
          for i in range(self.noOfRepeats):
              ...
              print 'the length of the scan list is '
              print len(self.scanList)
              for j in range(len(self.scanList)):
                  if(self.scanList[j][0]=='a'):
                      print 'angle scan'
                      #print str((self.scanList[j][1]), str(self.scanList[j][2]), str(self.scanList[j][3]), str(self.scanList[j][4])
    ...
    def incrementGUIRepeat(self):
          ...
          print 'sending scan complete from script'
          self.controller.update(None,scandata)
          self.mcontroller.update(None,scandata)
    ...
    def anglescan(self,start,end,step,collectionTime):
        ...
        try:
            for i in range(npoints):
                ...
                dcm_bragg.asynchronousMoveTo(currentpos)
                dcm_bragg.waitWhileBusy()
                ##ScannableBase.waitForScannable(dcm_mono)
                self.ionchamberData.append(self.ionchambers.collectData())
                # print out some progress
                print dcm_bragg.getPosition(),self.ionchamberData[i][0],self.ionchamberData[i][1],self.ionchamberData[i][2]             
            ...
            print "error in ae scan " , exception
            scandata=Vector()
            scandata.add("error")
            self.controller.update(None, scandata)
            self.mcontroller.update(None, scandata)
        ...
        self.tag=self.tag+1
        #self.controller.update(None, "ScanComplete")
        #self.mcontroller.update(None, "ScanComplete")
    ...
    def kscan(self,start,end,step,kStartTime,kEndTime,kWeighting, edgeEnergy, twoD):
        ...
        for i in range(npoints):
            ...
            dcm_bragg.asynchronousMoveTo(mdegPosition)
            dcm_bragg.waitWhileBusy()
            #ScannableBase.waitForScannable(dcm_mono)
    ...
    def writeSummary(self,npoints,start,end,step):
        ...
        for i in range(npoints):
           ...
           # print ionchamberData
            ...
            #print "Creating SDP..."
            detectorVector = []
            print self.ionchamberData
            #for ion in self.ionchamberData[i]:
            #    detectorVector.append(ion)
            iondata = jarray.array(self.ionchamberData[i], 'd')    
            #detectorVector.append(iondata)
            windowArray = jarray.array(windowedData, 'd')
            #detectorVector.append(windowArray)
            #detectorVector.append(jarray.array(self.ionchamberData[i][0:2],'d'))
            #detectorVector.append(jarray.array(windowedData,'d'))
            positionVector = []
            positionVector.append(current)
            ...
            print sdp
            #s = String.valueOf(self.fileno))
            #sdp.setScanIdentifier(s)
    ...
    def writeKScanSummary(self,npoints,start,end,step,edgeEnergy,twoD):
        ...
        for i in range(npoints):
            ...
            print "Creating SDP..."
            detectorVector = []
            detectorVector.append(self.ionchamberData[i])
            #detectorVector.append(jarray.array(self.ionchamberData[i][0:2],'d'))
            detectorVector.append(windowedData[0:])
            #detectorVector.append(jarray.array(windowedData,'d'))
            positionVector = []
            positionVector.append(str(current))
            #sdp = ScanDataPoint("scan name",self.scannableNamesVector,self.detectorNamesVector,positionVector,detectorVector,"Panel Name","I18 Custom SDP","Header String",self.mcaList[i])
            #sdp = ScanDataPoint("Exafs FluKScan",self.scannableNamesVector,self.detectorNamesVector,None,None,None,None,positionVector,detectorVector,None,"Panel Name","I18 Custom SDP","Header String",self.mcaList[i],0)
            sdp = ScanDataPoint()
            sdp.setUniqueName("Exafs FluKScan")
            for s in self.scannableNamesVector:
                sdp.addScannable(s)
            for d in self.detectorNamesVector:
                sdp.addDetector(d)
            for p in positionVector:
                sdp.addScannablePosition(p,["%.4f"] )
            sdp.addDetectorData(jarray.array(self.ionchamberData[i], 'd'),["%5.2g","%5.2g","%5.2g"] )
            sdp.addDetectorData(windowedData,["%5.2g","%5.2g","%5.2g","%5.2g","%5.2g","%5.2g","%5.2g","%5.2g","%5.2g"] )
            sdp.setCurrentFilename(self.datafilename)
    ...
    def mDegForK(self,k,edgeEnergy,twoD):
        return gda.gui.exafs.i18.Converter.convert(k,gda.gui.exafs.Converter.PERANGSTROM, gda.gui.exafs.Converter.MDEG,edgeEnergy, twoD)

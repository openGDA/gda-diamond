...
detVisitPath = "/dls/i15-1/data/2017/cm16757-1/processing/"
...
def detSetAreaDetector(det="det1"):
    ...
    ##tfm
    caput(detPVs[det]+"TFM:NDArrayPort","pe1.rx")
    ...
    ##stat1
    caput(detPVs[det]+"STAT:NDArrayPort","pe1.rx")
    caput(detPVs[det]+"STAT:EnableCallbacks","1")
    caput(detPVs[det]+"STAT:MinCallbackTime","0")
    caput(detPVs[det]+"STAT:BlockingCallbacks","0")
    caput(detPVs[det]+"STAT:ComputeStatistics","1")
    ##Set path for filewriters
    detSetPath(path=detVisitPath)
...
def detSetPath(path=detVisitPath):
    if path[:4] != "/dls" :
        path = detVisitPath + path

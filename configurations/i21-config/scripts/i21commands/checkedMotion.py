...
def findRange(demand):
    for key in lookuptable.keys():
        if (demand >= key[0] and demand < key[1]):
            return key
    return None
...
def checkIfMoveLegal(motor, new_position):
    from scannabledevices.pausableScannable_instances import armtth
    if motor is armtth:
        if not moveWithinLimits(float(motor.getPosition()), float(new_position)):
            ...
            find_range = findRange(new_position)
            ...
            from gdaserver import sgmr1, specl
...
def move(motor, new_position):
    checkIfMoveLegal(motor, new_position)

...
class StageDataCollection:
    ...
    def createSampleDict(self, calibrant='Si', calibrant_x=0, calibrant_y=0, calibrant_exposure=1.0, sample_x_start=0, sample_x_stop=1, sample_x_step=0.1, sample_y_start=None, sample_y_stop=None, sample_y_step=None, sample_exposure=1.0, pixium_x=None, pixium_y=None, pixium_z=None ):
        sample={}
    ...
    def doSampleDataCollection(self, sample_x_start=0, sample_x_stop=1, sample_x_step=0.1, sample_y_start=None, sample_y_stop=None, sample_y_step=None, sample_exposure=1.0):
        # sample diffraction data
        args=[self.stage_x,sample_x_start,sample_x_stop,sample_x_step ]
    ...
    def moveToSafePosition(self, stageName='ms1'):
        self.stage_x.moveTo(float(self.lookup_table[stageName.upper()][0]))
        self.stage_y.moveTo(float(self.lookup_table[stageName.upper()][1]))
        #self.stage_rot.moveTo(lookup_table[stageName.upper()][2])
            
    def isAtSafePosition(self, stageName='ms1'):
        return float(self.stage_x.getPosition())==float(self.lookup_table[stageName.upper()][0]) and float(self.stage_y.getPosition())==float(self.lookup_table[stageName.upper()][1])
...
sampledatacollection=StageDataCollection("sampledatacollection")
collectDiffractionData=sampledatacollection.collectData
moveToSafePosition=sampledatacollection.moveToSafePosition

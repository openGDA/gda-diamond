...
print "==================== I am git controlled script!!! =================="
class ScanCreator:
		...
		def create_group_and_tuples(self):
				...
				global scan_group
				# can't use globals() here if imported from localStation.py
				if "scan_group" not in __main__.__dict__:
						scan_group = ScannableGroup()
						self.insert_into_namespace("scan_group", scan_group,name_space=__main__.__dict__)
				else:
				...
				self.insert_into_namespace("scan_points", tuples, name_space=__main__.__dict__)
				return tuples

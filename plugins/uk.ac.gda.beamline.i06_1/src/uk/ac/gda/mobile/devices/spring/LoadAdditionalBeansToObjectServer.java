...
package uk.ac.gda.mobile.devices.spring;
...
/**
 * instance of this class provides method {@link #loadAdditionalBeans(String[])}to create and add additional
 ...
 * <li>use this instance's {@link #loadAdditionalBeans(String[])} to load bean definition file in a script to add new beans into GDA server</li>
...
public class LoadAdditionalBeansToObjectServer extends ApplicationObjectSupport implements Configurable, Findable {
	...
	public void loadAdditionalBeans(String[] filepaths) {
		...
		try {
			...
			InterfaceProvider.getTerminalPrinter().print(e.getMessage());
	...
	private void placeInJythonNamescape() {
		...
		for (String bean : beanDefinitionNames) {
			...
			if (bean2 instanceof Scannable) {
				InterfaceProvider.getJythonNamespace().placeInJythonNamespace(bean, bean2);
			}
		}
	}
	/**
	...
	private void addAdapterFactoryToFinder() {
		...
		for (Map.Entry<String, AdapterFactory> entry : adapterFactories.entrySet()) {
			...
			logger.info(
					String.format("Adding AdapterFactory %s (namespace %s) to finder", StringUtils.quote(name), StringUtils.quote(adapterFactory.getName())));
	...
	public void configure() throws FactoryException {
		InterfaceProvider.getJythonNamespace().placeInJythonNamespace(getName(), this);
	}
